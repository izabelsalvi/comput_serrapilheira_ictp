...
tapply(X = iris$Sepal.Length, INDEX = list(iris$Species), FUN = mean) #apply a function conditional to a particuar factor
...
aggregate(Sepal.Length ~ Species, data = iris, mean) #now we can notate de columns y= species and x = length


## NAs and zeroes --------------------------------------------------------------
apply(iris, 2, function(x) sum(is.na(x))) #definig the function inside the apply
?apply

summary(iris) #to see if i have any zeros
...
cv <- function(x){   ##coeficient of variation
  sd(x) / mean(x) * 100
}
apply(vars, 2, cv)
...
min(vars$Sepal.Length)
range(vars$Sepal.Length)
...
which(iris$Sepal.Width %in% outliers) #position of those values in my table
# let's use the position to index the object
iris[which(iris$Sepal.Width %in% outliers), c("Sepal.Width", "Species")]

#if i want to remove the outliers
iris_noout<- iris[which(!iris$Sepal.Width %in% outliers), c("Sepal.Width", "Species")]
...
par(mfrow = c(1, 1))
#check if the data follows a normal distribution looking ate how much the dots fits the line
